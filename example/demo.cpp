...
// void renderDemo(NVGcontext* vg, float mx, float my, float width, float
// height, float t, int blowup, DemoData* data);
...
DemoData::DemoData(NVGcontext *vg) : _vg(vg) {}
DemoData::~DemoData() {
  for (int i = 0; i < 12; i++) {
    nvgDeleteImage(_vg, _images[i]);
  }
}
...
bool DemoData::load() {
  ...
  for (int i = 0; i < 12; i++) {
    ...
    _images[i] = nvgCreateImage(_vg, file, 0);
    if (_images[i] == 0) {
  ...
  _fontIcons = nvgCreateFont(_vg, "icons", "../example/entypo.ttf");
  if (_fontIcons == -1) {
    printf("Could not add font icons.\n");
    return false;
  }

  _fontNormal = nvgCreateFont(_vg, "sans", "../example/Roboto-Regular.ttf");
  if (_fontNormal == -1) {
    printf("Could not add font italic.\n");
    return false;
  }

  _fontBold = nvgCreateFont(_vg, "sans-bold", "../example/Roboto-Bold.ttf");
  if (_fontBold == -1) {
    printf("Could not add font bold.\n");
    return false;
  }

  _fontEmoji = nvgCreateFont(_vg, "emoji", "../example/NotoEmoji-Regular.ttf");
  if (_fontEmoji == -1) {
    printf("Could not add font emoji.\n");
    return false;
  }

  nvgAddFallbackFontId(_vg, _fontNormal, _fontEmoji);
  nvgAddFallbackFontId(_vg, _fontBold, _fontEmoji);

  return true;
}

void DemoData::render(float mx, float my, float width, float height, float t,
                      int blowup) {
  float x, y, popy;

  drawEyes(_vg, width - 250, 50, 150, 100, mx, my, t);
  drawParagraph(_vg, width - 450, 50, 150, 100, mx, my);
  drawGraph(_vg, 0, height / 2, width, height / 2, t);
  drawColorwheel(_vg, width - 300, height - 300, 250.0f, 250.0f, t);

  // Line joints
  drawLines(_vg, 120, height - 50, 600, 50, t);
  ...
  drawScissor(_vg, 50, height - 80, t);

  nvgSave(_vg);
  if (blowup) {
    nvgRotate(_vg, sinf(t * 0.3f) * 5.0f / 180.0f * NVG_PI);
    nvgScale(_vg, 2.0f, 2.0f);
  }
  ...
  drawButton(_vg, ICON_LOGIN, "Sign in", x + 138, y, 140, 28,
  ...
  y += 25;
  drawEditBoxNum(_vg, "123.00", "px", x + 180, y, 100, 28);
  drawSlider(_vg, 0.4f, x, y, 170, 28);
  y += 55;

  drawButton(_vg, ICON_TRASH, "Delete", x, y, 160, 28, nvgRGBA(128, 16, 8, 255));
  drawButton(_vg, 0, "Cancel", x + 170, y, 110, 28, nvgRGBA(0, 0, 0, 0));

  // Thumbnails box
  drawThumbnails(_vg, 365, popy - 30, 160, 300, _images, 12, t);

  nvgRestore(_vg);
}

void DemoData::saveScreenShot(int w, int h, int premult, const char *name) {
