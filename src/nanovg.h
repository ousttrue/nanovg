...
enum NVGcreateFlags {
    ...
    // slower, but path overlaps (i.e. self-intersecting or sharp turns)
    // will be
    // drawn just once.
...
struct NVGtextRow {
    ...
    const char *end;    // Pointer to the input text where the row ends (one
                        // past the last character).
    const char *next;   // Pointer to the beginning of the next row.
    float width;        // Logical width of the row.
    float minx,
        maxx;  // Actual bounds of the row. Logical with and bounds can
...
enum NVGimageFlags {
    ...
    NVG_IMAGE_NEAREST =
        1 << 5,  // Image interpolation is Nearest instead Linear
};
...
// in relation currently set viewport (i.e. glViewport on GL backends).
...
// alpha components separately. The parameters should be one of
// NVGblendFactor.
...
// value, different kinds of paints can be created using
// nvgLinearGradient(), nvgBoxGradient(), nvgRadialGradient() and
// nvgImagePattern().
//
// Current render style can be saved and restored using nvgSave() and
// nvgRestore().

// Sets whether to draw antialias for nvgStroke() and nvgFill(). It's
// enabled by default.
...
// Already transparent paths will get proportionally more transparent as
// well.
...
// used for rendering. In addition you can upload your own image. The image
// loading is provided by stb_image. The parameter imageFlags is combination
// of flags defined in NVGimageFlags.
...
// current transform when it is passed to nvgFillPaint() or
// nvgStrokePaint().
...
// (w,h) define the size of the rectangle, r defines the corner radius, and
...
// gradient. The gradient is transformed by the current transform when it is
...
// transformed by the current transform when it is passed to nvgFillPaint()
// or nvgStrokePaint().
...
// angle rotation around the top-left corner, image is handle to the image
// to render. The gradient is transformed by the current transform when it
// is passed to nvgFillPaint() or nvgStrokePaint().
...
// which describe the shape. The are functions to draw common shapes like
// rectangles and circles, and lower level step-by-step functions, which
// allow to define a path curve by curve.
//
// NanoVG uses even-odd fill rule to draw the shapes. Solid shapes should
// have counter clockwise winding and holes should have counter clockwise
// order. To specify winding of a path you can call nvgPathWinding(). This
// is useful especially for the common shapes, which are drawn CCW.
//
// Finally you can fill the path using current fill style by calling
// nvgFill(), and stroke it with current stroke style by calling
// nvgStroke().
//
...
// style and by specifying the fill color. Common text and font settings
// such as font size, letter spacing and text align are supported. Font blur
// allows you to create simple text effects such as drop shadows.
//
...
// and rotation, the returned line height is always 16. Some measures may
// vary because of the scaling since aforementioned pixel snapping.
//
...
// drawn. White space is stripped at the beginning of the rows, the text is
...
// bounds value are [xmin,ymin, xmax,ymax] Returns the horizontal advance of
// the measured text (i.e. where the next character should drawn). Measured
...
// specified only the sub-string will be used. Measured values are returned
// in local coordinate space.
...
// rows, the text is split at word boundaries or when new-line characters
// are encountered. Words longer than the max width are slit at nearest
...
struct NVGparams {
    ...
    int (*renderUpdateTexture)(struct NVGparams *params, int image, int x,
                               int y, int w, int h,
                               const unsigned char *data) = {};
